...
// =====================================================================
// Driver "mock": store en memoria. Implementa la interfaz de datos de la
// app. El driver Prisma (prisma-repo.ts) expone exactamente la misma
// interfaz contra PostgreSQL; `index.ts` elige según DATA_DRIVER.
// =====================================================================
...
export const mockDb = {
  ...
  progress: {
    ...
    async statsForUser(userId: string) {
      const idx = await mockDb.courses.lessonIndex()
  ...
  admin: {
    async overview() {
      ...
      return {
        ...
        upcomingBookings: store.bookings.filter((b) => b.status === 'CONFIRMED').length,
      }
    },
